...
function resetForm_my_applied_req($form) {
    $form.find('input:text, input:password, input:file, textarea').val('');
    $form.find('input:radio, input:checkbox')
         .removeAttr('checked').removeAttr('selected');
    $('#sel_region_my_applied_req')[0].selectedIndex=0;
    $('#sel_department_my_applied_req')[0].selectedIndex=0;
    $('#sel_recruiter_my_applied_req')[0].selectedIndex=0;
}
function clearMySearchForm_my_applied_req() {
	resetForm_my_applied_req($('#mySearchForm_my_applied_req')); // by id, recommended
}
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="requirementController.do?search_my_applied_req" method="post" id="mySearchForm_my_applied_req">
		<div class="searchBar">
			<table class="searchContent" >
			    <tr>
			      <!-- <td colspan="5">&nbsp;<span style="font-weight:bold;color:#FF0000;">我申请的需求</span></td> -->

			    	<td>
				    	&nbsp; 地区：
				    	<%-- <input name="region_search" type="text" value="${region_search }" size="12" /> --%>
				    	<select name="region_search" class="" id="sel_region_my_applied_req" >
				    	...
				    	<select name="department_search" class="" id="sel_department_my_applied_req" >
				    		<option value="0">请选择</option>
			    ...
			    <tr>
				    ...
				    <td >
				    	...
				    	<select name="recruiter_search" class="" id="sel_recruiter_my_applied_req" >
							<option value="0">请选择</option>
				    	...
				    	&nbsp; &nbsp;<input type="submit" value="搜索" />&nbsp;&nbsp;&nbsp;<input type="button" value="清空重输" onclick="clearMySearchForm_my_applied_req()" />
				    </td>
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			...
			<!-- <li><a class="edit" href="requirementController.do?gotoModifyRequirement&req_id={reqid}" target="navTab" title="修改需求" ><span>修改</span></a></li>-->
		</ul>
			...
			<c:if test="${!empty requirements }">
				<c:forEach items="${requirements}" var="requirement">
					<tr target="reqid" rel="${requirement.id }">
						...
						<td>N/A</td>
						<%-- <td>${requirement.interview_num }</td> --%>
						<td>
