...
<script type="text/javascript">
	function start_requirement(url, data) {
		alertMsg.confirm("请确认是否启动？", {
			okCall : function() {
	...
	function resetForm_pau_req($form) {
	    $form.find('input:text, input:password, input:file, textarea').val('');
	    $form.find('input:radio, input:checkbox')
	         .removeAttr('checked').removeAttr('selected');
	    $('#sel_region_pau_req')[0].selectedIndex=0;
	    $('#sel_department_pau_req')[0].selectedIndex=0;
	    $('#sel_recruiter_pau_req')[0].selectedIndex=0;
	}
	function clearMySearchForm_pau_req() {
		resetForm_pau_req($('#mySearchForm_pau_req')); // by id, recommended
	}
	
</script>

<form id="pagerForm" rel="searchForm" method="post" action="requirementController.do?search_pau_req">
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="requirementController.do?search_pau_req" method="post"
		id="mySearchForm_pau_req">
		<div class="searchBar">
			<table class="searchContent">
				<tr>
			      ...
			      <!-- <td colspan="5">&nbsp;<span style="font-weight:bold;color:#FF0000;">已发布需求</span></td> -->
			    	<td>
				    	...
				    	<select name="region_search" class="" id="sel_region_pau_req" >
				    	...
				    	<select name="department_search" class="" id="sel_department_pau_req" >
				    		<option value="0">请选择</option>
				...
				<tr>
				    ...
				    <td >
				    	...
				    	<select name="recruiter_search" class="" id="sel_recruiter_pau_req" >
							<option value="0">请选择</option>
				    	...
				    	&nbsp; &nbsp;<input type="submit" value="搜索" />&nbsp;&nbsp;&nbsp;<input type="button" value="清空重输" onclick="clearMySearchForm_pau_req()" />
				    </td>
		...
		<ul class="toolBar">
			...
			<c:if test="${sessionScope.user.rname  == '招聘主管' || sessionScope.user.rname  == '系统管理员'}">
				<li><a class="edit"
					...
					target="navTab" rel="modify_requirement_tab" navTabId="modify_requirement_tab" title="修改需求"><span>修改</span></a></li>
			</c:if>
		</ul>
					...
					<tr target="reqid" rel="${requirement.id }">
						...
						<td><c:if test="${sessionScope.user.rname  == '招聘主管' || sessionScope.user.rname  == '系统管理员'}">
								<%-- <a href="javascript:;"
									...
									onclick="start_requirement('requirementController.do?start_requirement_json&req_id=${requirement.id }')">启动</a> --%>
								<a href="requirementController.do?start_requirement_json&req_id=${requirement.id }"
									style="text-decoration:none;font-weight:bold;color:#FF0000;"
									target="ajaxTodo" title="确定要启动吗？" >启动</a>
							</c:if></td>
					</tr>
				</c:forEach>
			</c:if>
