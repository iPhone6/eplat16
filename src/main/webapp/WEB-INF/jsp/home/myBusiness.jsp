...
<script type="text/javascript">
	...
	function clearMySearchForm() {
		//debugger;
		// to call, use:
		resetForm($('#mySearchForm')); // by id, recommended
	}
</script>
<%-- 
...
<form id="pagerForm" rel="searchForm" method="post"
	action="candidateController.do?search">
	...
	<input type="hidden" name="mobilePhone" type="text"
		value="${mobilePhone }" /> <input type="hidden" name="channel"
		type="text" value="${channel }" /> <input type="hidden"
		name="position" type="text" value="${position }" /> <input
		type="hidden" name="status" type="text" value="${status }" />
	...
	<input type="hidden" type="text" name="interviewTime_start"
		value="${interviewTime_start }"> <input type="hidden"
		type="text" name="interviewTime_end" value="${interviewTime_end }">
</form>
...
<div class="pageHeader">
	...
	<form onsubmit="return navTabSearch(this);" id="mySearchForm"
		action="candidateController.do?search" method="post">
		<div class="searchBar">
			<table class="searchContent">
				...
				<tr>
					<td>&nbsp; 姓名：<input name="name" type="text" value="${name }" />
						&nbsp;身份证：<input name="identityNo" type="text"
						value="${identityNo }" /> &nbsp;手机号：<input name="mobilePhone"
						type="text" value="${mobilePhone }" /> &nbsp;渠道来源：<input
						name="channel" type="text" value="${channel }" /> &nbsp;状态： <select
						name="status" id="status_sel">
				...
				<tr>
					<td>&nbsp; 岗位：<input name="position" type="text"
						value="${position }" /> &nbsp; 面试时间： <!-- <input type="text" name="date2" class="date" minDate="2000-01-15" maxDate="2012-12-15" readonly="true"/><a class="inputDateButton" href="javascript:;">选择</a> -->
						...
						type="button" value="清空重输" onclick="clearMySearchForm()" />
					</td>
				</tr>
			</table>
		</div>
	...
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="edit" href="resumeController.do?importResume"
				target="dialog" mask="true" rel="my_candidates_t" title="简历导入"
				fresh="false"><span>导入简历</span></a></li>
			<li><a class="edit" href="resumeController.do?importResume2"
				target="dialog" mask="true" rel="my_candidates_t" title="多简历导入"
				fresh="false"><span>多简历导入</span></a></li>

			<li><a class="edit"
				href="resumeController.do?updateResume&rid={rid}&cb=hxrsf"
			...
			<li><a class="edit"
				href="offerController.do?insertOffer&rid={rid}"
				target="dialog" mask="true"><span>发起Offer申请</span></a></li>
			<!-- 
				<li>
			...
			<c:if test="${!empty candidates }">
				<c:forEach items="${candidates}" var="candidate">
					<tr target="rid" rel="${candidate.rid }">
						...
						<td>
							<!-- 注1：这里a标签的target属性的值设为navTab时，点击链接后，就不会刷新整个页面，而是在当前页面上新增加一个页签来显示页面跳转后的内容！ -->
							<!-- 注2：这里a标签的rel属性相当于给当前用于显示数据的页签设定了一个id，当点击了这个链接后，它就会在所有页签中找到具有该页签id的页签，将链接返回来的页面数据显示在这个页签上，从而避免了点击链接后不会跳到其他不相干的页签或页面去了！ -->
							<!-- 注3：这里a标签的title属性的主要作用就是，给当前页签设定一个标题名字，显示在该页签上，起到页签标题的作用。如果没有指定title属性，则会默认以该链接的名字来命名页签！ -->
							<a
							href="resumeController.do?updateResume&rid=${candidate.rid }&cb=cb&cid=${candidate.cid }"
							target="dialog" rel="my_candidates_t" mask="true" title="我的候选人"
							style="text-decoration:none;font-weight:bold;color:#FF0000;">储备</a>
							<a
