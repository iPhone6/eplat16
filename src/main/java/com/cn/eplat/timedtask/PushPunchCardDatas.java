...
//	}
	public static boolean isIs_busy_finding_missing_not_uploaded_punch_card_datas() {
		return is_busy_finding_missing_not_uploaded_punch_card_datas;
	}
	public static void setIs_busy_finding_missing_not_uploaded_punch_card_datas(boolean is_busy_finding_missing_not_uploaded_punch_card_datas) {
		PushPunchCardDatas.is_busy_finding_missing_not_uploaded_punch_card_datas = is_busy_finding_missing_not_uploaded_punch_card_datas;
	}
...
//	public static PushPunchCardDatas getInstanceOfPushPunchCardDatas(){
//		return this;
//	}
	
//	@Scheduled(cron = "${push_punch_card_datas.schedule}")	// 按预先设定的时间间隔执行push方法
//	@Scheduled(cron = "0/20 * * * * ? ")	// （快速测试用定时设置。。。）
	public void push() {
					...
					if(log_time.after(tolerant_time)) {	// 如果打卡机系统日志时间晚于允许的误差时间，则认为这条系统日志时间明显有误
						...
						if(StringUtils.isBlank(log_descr)) {
							...
							if(log_descr.contains("下载记录")) {	// 如果打卡机系统日志描述信息中包含“下载记录”关键字（即对应“从设备下载记录数据”这个日志描述信息），则表示需要开始查找遗漏未上传阿里云的打卡机打卡数据
								...
								msc.setStatus("已处理");
								msc.setProc_result("正在查找是否有遗漏未上传阿里云服务器的打卡数据中...");
...
//								int insert_all_ret = machCheckInOutService.batchInsertAllAccessCheckinoutsToMySQLMachCheckInOutCopy(all_pc_datas);
			...
			myTimer.schedule(myTimerTask, my_timer_delay);
...
//											mcio.setPush_status("push_success");
									...
									if(desc.contains("repush")) {
										...
										if(batch_mod_ret <= 0) {
											logger.error("(repush success)-批量修改打卡机(" + sn + ")打卡数据推送状态出现异常，batch_mod_ret = " + batch_mod_ret);
										} else {
											logger.info("(repush success)-批量修改打卡机(" + sn + ")打卡数据推送状态成功，batch_mod_ret = " + batch_mod_ret);
										}
									} else {
										int batch_add_ret = 0;
										DataSourceContextHolder.setDbType(DataSourceType.SOURCE_ADMIN);
										batch_add_ret = machCheckInOutService.batchAddMachCheckInOut(mcios_to_add);
										
										if(batch_add_ret <= 0) {
											logger.error("(" + desc + " success)-批量添加打卡机(" + sn + ")打卡数据出现异常，batch_add_ret = " + batch_add_ret);
										} else {
											logger.info("(" + desc + " success)-批量添加打卡机(" + sn + ")打卡数据成功，batch_add_ret = " + batch_add_ret);
										}
									}
